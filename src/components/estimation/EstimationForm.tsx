...
export function EstimationForm({
	...
	const settings: EstimationSettings = useMemo(
		() => parseSettings(settingRows),
		[settingRows],
	);
	...
	const [overridePrice, setOverridePrice] = useState<number>(
		existing?.overrides?.final_price ?? 0,
	);
	...
	function setInput<K extends keyof EstimationInputs>(
		key: K,
		value: EstimationInputs[K],
	) {
	...
	async function handleSave() {
		...
		try {
			...
			const overrides = overridePrice > 0
				? {
			...
			const payload = {
				...
				settings_snapshot:
					settingsSnapshot as unknown as import("@/lib/supabase/types").Json,
				outputs: outputs as unknown as import("@/lib/supabase/types").Json,
				overrides: overrides as unknown as
						...
						<tbody className="divide-y divide-gray-100 dark:divide-gray-800">
							...
							{inputs.toll_estimate > 0 && (
								<BreakdownRow
									...
									value={outputs.toll_cost}
								/>
							)}
							{inputs.other_cost > 0 && (
								<BreakdownRow
									label={tEstBreakdown("other")}
									value={outputs.other_cost}
								/>
							)}
							<BreakdownRow
								label={tEstBreakdown("jobCost")}
								value={outputs.job_cost}
								bold
							/>
							<BreakdownRow
							...
							/>
							<BreakdownRow
								label={tBreakdown("adjustedCost")}
								value={outputs.adjusted_cost}
								bold
							/>
							<BreakdownRow
							...
							/>
							<BreakdownRow
								label={tBreakdown("finalProfit")}
								value={outputs.final_profit}
							/>
							<BreakdownRow
							...
							<BreakdownRow
								...
								value={
									outputs.initial_offer_price - outputs.internal_target_price
								}
								muted
							/>
						</tbody>
