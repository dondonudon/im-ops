	...
	return (
		<div className="space-y-4">
			...
			<Card className="divide-y divide-line">
				{assignments.length === 0 && <EmptyState title={t("empty")} />}
				{assignments.map((a) => {
					const name = a.assignment_type === "fleet" ? a.fleet?.name : a.crew?.name;
					return (
						<div key={a.id} className="flex items-center justify-between px-4 py-3 text-sm">
							<div>
								<span className="font-medium text-ink">{name ?? "—"}</span>
								{a.role && <span className="text-ink-faint text-xs ml-2">({a.role})</span>}
								<span className="ml-2 text-xs text-primary-text">
									{a.assignment_type === "fleet" ? t("fleetLabel") : t("crewLabel")}
								</span>
							</div>
							<button
								type="button"
			...
			{showModal && (
				...
				>
					<div className="w-full max-w-sm bg-surface border border-line rounded-xl p-6 shadow-token-md space-y-4">
						<h3 id="assign-title" className="text-base font-semibold text-ink">
							{t("addAssignment")}
						</h3>
										...
										{(["fleet", "crew"] as const).map((opt) => (
											<label key={opt} className="flex items-center gap-1.5 cursor-pointer">
												<input
													...
													onChange={() => setForm((p) => ({ ...p, type: opt, resourceId: "" }))}
													className="text-primary focus:ring-[var(--ring)]"
												/>
												<span className="text-sm text-ink">
													{opt === "fleet" ? t("fleetLabel") : t("crewLabel")}
												</span>
											</label>
										))}
									</div>
